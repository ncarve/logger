...
const logLevels = [
  ...
  {level: "highlight", display:"hligt", color: "fgMagenta"}
];

const printLog = (level, ...msgs) => {
  const logInfo = (level) => {
    const rank = R.findIndex(R.compose(R.equals(level), l => l.level), logLevels);
    return {rank, level, display: logLevels[rank].display, color: nodeColors[logLevels[rank].color]};
  };
  const li = logInfo(level);
  if (li.rank >= logInfo(logThreshold).rank) {
    const reset = nodeColors["reset"];
    const maxLength = R.reduce((cur, ll) => R.max(cur, (ll.display || ll.level).length), 0, logLevels)
    const logLevelDisplay = (li.display || li.level).toUpperCase().padEnd(maxLength);
